...
//     createDate: /166\d{10}/,
//     toc: [
//       { name: "概述", aside: "", anchor: "article-md-title-1" },
//       {
//         name: "简单请求",
//         aside: "",
//         anchor: "article-md-title-2",
//         children: [
//           { name: "简单请求", aside: "", anchor: "article-md-title-6" },
//           { name: "简单请求", aside: "", anchor: "article-md-title-7" },
//           { name: "简单请求", aside: "", anchor: "article-md-title-8" },
//         ],
//       },
//       { name: "概述", aside: "", anchor: "article-md-title-3" },
//       { name: "概述", aside: "", anchor: "article-md-title-4" },
//       { name: "概述", aside: "", anchor: "article-md-title-5" },
//     ],
//     "thumb|1": [
//       Mock.Random.image("300x250", "#888", "#fff", "Random Image"),
//       null,
//     ],
//     htmlContent: `<p><br></p><h3 style="text-align: left; line-height: 1.5;">一、背景</h3><ol><li style="text-align: left;">前端开发需要依赖后端接口</li><li style="text-align: left;">后端接口输出慢、接口规范随时可能会变，而前端毫无感知</li><li style="text-align: left;">前端需要自己 mock 假数据 json 文件</li><li style="text-align: left;">假数据 json 数据内容是静态的，测试不同返回情况需要修改 json 文件<br>...</li></ol><p style="text-align: left;">因此我们需要一种可以帮我们构造数据的工具，并解决以上的若干痛点<br>mock.js 是一个不错的工具</p><p style="text-align: left;"><strong>本文 github mock 使用完整例子</strong></p><p><br></p>`,
//   },
// });
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
  ...
  <p>JSONP 并不是一个好的跨域解决方案，它至少有着下面两个严重问题：</p><ol>
  ...
  <p>所以，CORS 是一种更好的跨域解决方案。</p><h1 id="article-md-title-1">概述</h1><p><code>CORS</code>是基于<code>http1.1</code>的一种跨域解决方案，它的全称是<strong>C</strong>ross-<strong>O</strong>rigin <strong>R</strong>esource <strong>S</strong>haring，跨域资源共享。</p><p>它的总体思路是：<strong>如果浏览器要跨域访问服务器的资源，需要获得服务器的允许</strong></p><figure class="markdown-image">
      ...
      </figure><p>而要知道，一个请求可以附带很多信息，从而会对服务器造成不同程度的影响</p><p>比如有的请求只是获取一些新闻，有的请求会改动服务器的数据</p><p>针对不同的请求，CORS 规定了三种不同的交互模式，分别是：</p><ul>
  <li><strong>简单请求</strong></li>
  <li><strong>需要预检的请求</strong></li>
  <li><strong>附带身份凭证的请求</strong></li>
  </ul>
  <p>这三种模式从上到下层层递进，请求可以做的事越来越多，要求也越来越严格。</p><p>下面分别说明三种请求模式的具体规范。</p><h1 id="article-md-title-2">简单请求</h1><p>当浏览器端运行了一段 ajax 代码（无论是使用 XMLHttpRequest 还是 fetch api），浏览器会首先判断它属于哪一种请求模式</p><h2 id="article-md-title-3">简单请求的判定</h2><p>当请求<strong>同时满足</strong>以下条件时，浏览器会认为它是一个简单请求：</p><ol>
  ...
  <span class="hljs-comment">// content-type 不满足要求，不是简单请求</span>
  fetch(<span class="hljs-string">"http://crossdomain.com/api/news"</span>, {
    <span class="hljs-attr">method</span>: <span class="hljs-string">"post"</span>,
    <span class="hljs-attr">headers</span>: {
  ...
  <h2 id="article-md-title-4">简单请求的交互规范</h2><p>当浏览器判定某个<strong>ajax 跨域请求</strong>是<strong>简单请求</strong>时，会发生以下的事情</p><ol>
  ...
  消息体中的数据</code></pre><p>当浏览器看到服务器允许自己访问后，高兴的像一个两百斤的孩子，于是，它就把响应顺利的交给 js，以完成后续的操作</p><p>下图简述了整个交互过程</p><figure class="markdown-image">
  ...
  添加用户成功</code></pre><p>可以看出，当完成预检之后，后续的处理与简单请求相同</p><p>下图简述了整个交互过程</p><figure class="markdown-image">
      ...
      </figure><h1 id="article-md-title-6">附带身份凭证的请求</h1><p>默认情况下，ajax 的跨域请求并不会附带 cookie，这样一来，某些需要权限的操作就无法进行</p><p>不过可以通过简单的配置就可以实现附带 cookie</p><pre><code class="language-js"><span class="hljs-comment">// xhr</span>
  ...
  <p>这样一来，该跨域的 ajax 请求就是一个<em>附带身份凭证的请求</em></p><p>当一个请求需要附带 cookie 时，无论它是简单请求，还是预检请求，都会在请求头中添加<code>cookie</code>字段</p><p>而服务器响应时，需要明确告知客户端：服务器允许这样的凭据</p><p>告知的方式也非常的简单，只需要在响应头中添加：<code>Access-Control-Allow-Credentials: true</code>即可</p><p>对于一个附带身份凭证的请求，若服务器没有明确告知，浏览器仍然视为跨域被拒绝。</p><p>另外要特别注意的是：<strong>对于附带身份凭证的请求，服务器不得设置 <code>Access-Control-Allow-Origin 的值为*</code></strong>。这就是为什么不推荐使用*的原因</p><h1 id="article-md-title-7">一个额外的补充</h1><p>在跨域访问时，JS 只能拿到一些最基本的响应头，如：Cache-Control、Content-Language、Content-Type、Expires、Last-Modified、Pragma，如果要访问其他头，则需要服务器设置本响应头。</p><p><code>Access-Control-Expose-Headers</code>头让服务器把允许浏览器访问的头放入白名单，例如：</p><pre><code>Access-Control-Expose-Headers: authorization, a, b</code></pre><p>这样 JS 就能够访问指定的响应头了。</p>`,
  },
});
...
Mock.mock("/api/comment", "post", {
  ...
  data: {
    ...
    createDate: new Date(),
    "avatar|1": [
      "http://n.sinaimg.cn/sinakd20113/537/w690h647/20200315/6de3-iquxrui5208326.jpg",
      "http://n.sinaimg.cn/sinakd20113/542/w690h652/20200315/cea9-iquxrui5208069.jpg",
      "https://www.mp3pc.com/uploads/6dad9a8b40eaab14d1066f0a019a9188.jpg",
      "https://i01piccdn.sogoucdn.com/84dc0c47fb7537d5",
    ],
  },
});
